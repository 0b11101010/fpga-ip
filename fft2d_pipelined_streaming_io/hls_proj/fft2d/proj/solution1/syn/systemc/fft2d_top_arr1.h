...
#ifndef _fft2d_top_arr1_H_
#define _fft2d_top_arr1_H_

#include <systemc>
#include <iostream>
#include <fstream>

#include "fft2d_top_arr1_memcore.h"

SC_MODULE(fft2d_top_arr1) {
...
protected:
    ...
    // buffer instances
    fft2d_top_arr1_memcore* buffer[BufferCount];
...
public:
    ~fft2d_top_arr1();
    SC_CTOR(fft2d_top_arr1) : m_trace_file(0) {
        for (unsigned i = 0; i < BufferCount; i++) {
            char szTmp[128];
            sprintf(szTmp, "%d", i);
            std::string name = "fft2d_top_arr1_memcore_U_";
            name += szTmp;

            buffer[i] = new fft2d_top_arr1_memcore(name.c_str());
            buffer[i]->clk      ( clk );
        ...
        sensitive << i_ce0  << i_address0;
        sensitive << t_ce0  << t_address0;
        ...
        if (dump_vcd) {
            m_trace_file = sc_create_vcd_trace_file( "fft2d_top_arr1.trace");
            // ports
